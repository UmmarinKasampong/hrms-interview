...
@section('title', 'ฟอร์มขอทำงานนอกสถานที่')
...
@section('contents')
    ...
    <div class="card  ms-auto me-auto mt-5" style="max-width: 1170px; width: 100%;">
        <div  class="fs-5 d-flex justify-content-between align-items-center card-header  text-white {{ $data[0]->status_form === 'Wait Progress' ? 'bg-info' : ($data[0]->status_form === 'Reject' ? 'bg-danger' : 'bg-success') }}">
            ขอทำงานนอกสถานที่ <a class="btn btn-primary">{{ $data[0]->status_form }}</a>
        </div>
                ...
                <div class="mb-3">
                    <div class="col-6">
                        ...
                        @if ($data[0]->offsite_path)
                            <div class="d-flex">
                                <a href="{{ url('storage/uploads/outDoc/' . $data[0]->offsite_path) }}"
    ...
    <script>
        ...
        function SendData(status) {
            ...
            let id_form = {!! json_encode($data[0]->offsite_id) !!}
